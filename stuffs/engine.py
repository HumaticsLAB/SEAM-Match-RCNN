...
import time
from models.match_head import MatchLossDF2, MatchLossWeak, NEWBalancedAggregationMatchLossWeak\
...
def train_one_epoch_reg(model, optimizer, data_loader, device, epoch, print_freq):
    if 'WORLD_SIZE' in os.environ and int(os.environ['WORLD_SIZE']) > 1:
        distributed = True
        rank = dist.get_rank()
    else:
    ...
    # if distributed:
    #     dist.barrier()
    real_model = model if not distributed else model.module
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header, rank=rank):
        ...
        if len(roi_features) > 0:
            ...
            loss_dict = {'match_loss': match_loss(logits, types, prod_ids, img_ids)}
...
def train_one_epoch_videomf(model, optimizer, data_loader, device, epoch, print_freq
...
def train_one_epoch_videonap_df2(model, optimizer, data_loader, device, epoch, print_freq
    ...
    tracking_predictor = real_model.roi_heads.tracking_predictor
    temporal_aggregator = real_model.roi_heads.temporal_aggregator
    match_loss = MatchLossDF2(device)
